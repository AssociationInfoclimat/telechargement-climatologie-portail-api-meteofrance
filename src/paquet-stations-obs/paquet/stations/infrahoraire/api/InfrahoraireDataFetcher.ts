...
export class InfrahoraireDataFetcher {
    ...
    async fetchInfrahoraireData(infrahoraireDate: InfrahoraireDate): Promise<InfrahoraireLine[]> {
        const response = await this.fetchInfrahoraireDataAPI(infrahoraireDate);
        ...
        if ([500, 502].includes(response.code)) {
            ...
            return await maker.fetchInfrahoraireData(infrahoraireDate);
        }
        if (response.code !== 200) {
            throw new UnexpectedResponseError(response);
        }
        return z.array(this.infrahoraireLineSchema).parse(response.data);
    }
}

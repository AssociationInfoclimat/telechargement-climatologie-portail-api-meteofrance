...
export function fetchListeStationsQuotidienne(departement: Departement): Promise<APIResponse<ListeStationsData>> {
    return getMF({
...
export class ListeStationsQuotidienneFetcher {
    async fetchListeStationsQuotidienne(
        ...
        const response = await fetchListeStationsQuotidienne(departement);
        ...
        if (response.code === 401) {
            ...
            return await this.fetchListeStationsQuotidienne(departement, { retries });
        }

...
export class InfrahoraireDataFetcher {
    ...
    async fetchInfrahoraireData(idStation: IdStation): Promise<InfrahoraireLine[]> {
        const response = await this.fetchInfrahoraireDataAPI(idStation);
        ...
        if ([500, 502].includes(response.code)) {
            ...
            return await maker.fetchInfrahoraireData(idStation);
        }

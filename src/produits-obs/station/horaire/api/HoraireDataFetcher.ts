...
export class HoraireDataFetcher {
    ...
    async fetchHoraireData({
        ...
        const response = await this.fetchHoraireDataAPI({ idStation, horaireDate });
        ...
        if ([500, 502].includes(response.code)) {
            ...
            const maker = new HoraireDataFetcher({
                ...
                horaireLineSchema: this.horaireLineSchema,
            });
            return await maker.fetchHoraireData({ idStation, horaireDate });
        }
        if (response.code !== 200) {
            throw new UnexpectedResponseError(response);
        }
        return z.array(this.horaireLineSchema).nonempty().parse(response.data)[0];
    }
}

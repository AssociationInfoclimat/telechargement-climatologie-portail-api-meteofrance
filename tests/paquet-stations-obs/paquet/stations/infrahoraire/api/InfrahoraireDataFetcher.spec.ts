...
} from '@/paquet-stations-obs/paquet/stations/infrahoraire/api/adapters/fetchInfrahoraireData.in-memory.js';
...
describe('InfrahoraireDataFetcher', () => {
        ...
        it('should throw too many retries error', async () => {
            const fetcher = new InfrahoraireDataFetcher({
                fetchInfrahoraireDataAPI: createInMemoryInfrahoraireDataAPIFetcher({
                    '2000-06-15T12:06:00Z': createServerErrorAPIResponse(),
                }),
                waitingTimeInMs: 0,
            });
            await expect(() =>
                fetcher.fetchInfrahoraireData(
                    createInfrahoraireDate({ year: 2000, month: 6, day: 15, hour: 12, minute: 6 })
                )
    ...
    describe('when unknown code', () => {
        it('should throw unexpected response error', async () => {
            const fetcher = new InfrahoraireDataFetcher({
                fetchInfrahoraireDataAPI: createInMemoryInfrahoraireDataAPIFetcher({
                    '2000-06-15T12:06:00Z': { code: 429, message: '', data: null },
                }),
            });
            await expect(() =>
                fetcher.fetchInfrahoraireData(
    ...
    describe('when wrong data', () => {
        it('should throw a zod error', async () => {
            const fetcher = new InfrahoraireDataFetcher({
                fetchInfrahoraireDataAPI: createInMemoryInfrahoraireDataAPIFetcher({
                    '2000-06-15T12:06:00Z': createSuccessfulAPIResponse([{ key: 'value' }]),
                }),
            });
            await expect(() =>
                fetcher.fetchInfrahoraireData(
    ...
    describe('when successful', () => {
        it('should return the data', async () => {
            ...
            const fetcher = new InfrahoraireDataFetcher({
                fetchInfrahoraireDataAPI: createInMemoryInfrahoraireDataAPIFetcher({
                    '2000-06-15T12:06:00Z': createSuccessfulAPIResponse([data]),
                }),
            });
            ...
            expect(fetched).toEqual([
                {
                    ...
                    ddraf10: WindDirection.of(PositiveInteger.of(160)),
                    raf10: PositiveFloat.of(11.1),

...
} from '@/paquet-obs/paquet/horaire/api/adapters/fetchHoraireData.in-memory.js';
...
describe('HoraireDataFetcher', () => {
    describe('when too many retries', () => {
        it('should throw too many retries error', async () => {
            ...
            await expect(() => fetcher.fetchHoraireData(Departement.of(76))).rejects.toThrow(TooManyRetriesError);
        });
    });
    describe('when unknown code', () => {
        it('should throw unexpected response error', async () => {
            const fetcher = new HoraireDataFetcher({
                fetchHoraireDataAPI: createInMemoryHoraireDataAPIFetcher({
                    76: { code: 429, message: '', data: null },
                }),
            });
            await expect(() => fetcher.fetchHoraireData(Departement.of(76))).rejects.toThrow(UnexpectedResponseError);
        });
    });
    describe('when wrong data', () => {
        it('should throw a zod error', async () => {
            ...
            await expect(() => fetcher.fetchHoraireData(Departement.of(76))).rejects.toThrow(ZodError);
        });
    });
    describe('when successful', () => {
        it('should return the data', async () => {
            ...
            const fetched = await fetcher.fetchHoraireData(Departement.of(76));
            expect(fetched).toEqual([
                {
                    ...
                    ddraf: WindDirection.of(PositiveInteger.of(160)),
                    raf: PositiveFloat.of(11.1),
